// Package sdkapi is the extension runtime contract: the service every
// out-of-process extension serves, so the daemon can learn what the extension
// declares and tell it when to initialize.
//
// It is written Go-first like an extension point -- the [Extension] interface
// and its message structs here are the source of truth, and this package stays
// free of any protobuf/gRPC imports -- but it is not a point: no extension
// provides it and the broker never resolves it. It is the fixed protocol the
// framework itself speaks, served by the SDK on every extension's socket and
// called by the host's launcher.
//
