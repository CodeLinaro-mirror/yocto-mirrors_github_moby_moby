...
// provider builds the caller for the daemon's provider of point over the
// callback connection. The callback channel serves one provider per point, so
// there is no enumeration to do beyond it.
...
// runtimeServer serves the extension runtime contract for this server. It is
// the plain Go implementation of [sdkapi.Extension]; the generated wiring puts
// it on the wire.
