...
A point has one hand-written Go contract, one hand-written generate file, and generated wire files:

```
extpoints/<area>/<name>/v0/
  <name>.go                 # interface, messages, Point, helpers
  gen.go                     # package doc and //go:generate
...
- Call `Point.Single`, `Point.All`, or `Point.ByExtension` from a helper rather than from engine code.
  ...
  Built-ins yield to installed providers and `extensions.Decide` falls back on failure.
...
Create `gen.go` beside the contract.
The directive is identical for every point; there are no paths to adjust:
...
- [ ] Put the hand-written contract and `gen.go` under `extpoints/<area>/<name>/v<N>/`.
...
| Wire it | `extpoints/<area>/<name>/v0/gen.go` | package doc and identical `//go:generate` |
