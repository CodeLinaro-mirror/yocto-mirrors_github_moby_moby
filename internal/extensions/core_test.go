...
// TestSingleSelection locks down the selection semantics of Point.Single, which
// live here rather than in any resolver: origin precedence, the one-provider
// limit, and error attribution.
...
// TestEffectiveProviders pins the masking rule every typed accessor resolves
// through.
...
// stubResolver is a Resolver over a fixed provider list.
...
// caller is the provider interface the point tests fan out over.
