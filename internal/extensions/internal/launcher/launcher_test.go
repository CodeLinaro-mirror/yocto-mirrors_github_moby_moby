...
// TestStopProcessSignalledExitIsNotAnError covers a runtime that takes the
// default action for the shutdown signal rather than handling it. Such a
// process exits with "signal: terminated", which is the stop the launcher
// asked for -- reporting it made every ordinary daemon shutdown log an error
// for an extension that did nothing wrong.
...
// TestStopProcessAfterSelfExit covers a child that exited on its own -- a
// crash, a panic -- while the daemon kept running. The launcher waits for it
// from the moment it starts, so it is reaped rather than left a zombie, and the
// later stop reads that result instead of calling Wait a second time.
...
// TestServicesRequireDeclaringThePoint covers a hand-written runtime that lists
// services under a point it did not declare it provides. The handshake carries
// the two independently, so without this check the host would publish those
// names on the API socket past the explicit opt-in that service.grpc exists to
// be. Extensions are trusted code, so this is a policy bypass rather than an
// escalation -- but the policy belongs where the declaration is read.
func TestServicesRequireDeclaringThePoint(t *testing.T) {
	...
	// Services under a point the extension did declare are accepted.
