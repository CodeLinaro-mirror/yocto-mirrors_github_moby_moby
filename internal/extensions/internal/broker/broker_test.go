...
// TestBuiltinEnumeration locks down the broker's part of replaceability: the
// registry records each extension's origin at registration and enumerates
// every provider flagged with it, taking no selection decision itself --
// origin precedence belongs to [extensions.EffectiveProviders], which
// single-provider resolution applies.
func TestBuiltinEnumeration(t *testing.T) {
	...
	t.Run("a masked builtin stays reachable by id", func(t *testing.T) {
		// Naming an extension explicitly outranks defaulting, so a dependent that
		// wants the stock behavior specifically can still get it.
		b := New()
		assert.NilError(t, b.RegisterBuiltin(stockExt))
		assert.NilError(t, b.Register(customExt))
