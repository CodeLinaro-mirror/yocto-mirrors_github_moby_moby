...
//
// It is safe for concurrent use: the read side (Provider,
...
// Register registers an installed extension: one that arrived from outside the
// host's own program, such as a launched binary. See [Broker.RegisterBuiltin]
// for the distinction origin makes.
func (b *Broker) Register(ext extensions.Extension) error {
	return b.register(ext, false)
}

// RegisterBuiltin registers a built-in extension: one the host's own program
// ships. Origin is the registrar's knowledge, not the extension's -- no
// declaration claims built-in-ness -- and it is what single-provider
// resolution's precedence is decided from: a built-in provider yields to an
// installed one (see [extensions.EffectiveProviders]).
...
// the lock (the exported readers, and resolveOrder during Init). It enumerates
// every registered provider, built-ins included: which of them a caller gets
// is selection, and selection is the typed point accessors' job (see
// [extensions.EffectiveProviders]), not the registry's.
