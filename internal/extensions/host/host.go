...
func New(ctx context.Context, opts Options) (_ *Host, retErr error) {
	...
	// Every provider is registered, so the single-provider constraint can be
	// checked across in-process and launched extensions alike. The constraint
	// has one source: the point contracts themselves, whose generated client
	// wiring carries each DefineSinglePoint declaration here.
...
// broker -- launched and in-process alike, since a launched extension's
// declaration carries a Shutdown that stops its process -- then any launched
// process the broker did not stop, then the dependency callback server.
//
// The second sweep is a backstop for a process that never reached the broker:
// one launched but not initialized, because an earlier extension in the
// dependency order failed. Stopping a process twice is a no-op.
...
func extensionFromLaunched(launched *launcher.Launched, providers map[extensions.PointID]clientpoint.Provider, exposeOnly map[extensions.PointID]bool) (extensions.Extension, error) {
	decl := extensions.Declaration{
		...
		// Shutdown stops the process, so the broker tears a launched extension
		// down in reverse dependency order alongside the in-process ones rather
		// than the host stopping every process afterwards. That ordering is the
		// contract: an extension's dependencies must outlive it, and a launched
		// extension runs its own Shutdown when the process is signalled -- if the
		// broker had already torn down an in-process provider it depends on, that
		// hook would be calling something that has stopped.
