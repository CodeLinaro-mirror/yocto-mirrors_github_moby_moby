...
func NewDaemon(ctx context.Context, config *config.Config, pluginStore *plugin.Store, authzMiddleware *authorization.Middleware, containerdDialer ContainerdDialer) (_ *Daemon, retErr error) {
	...
	// Built after the cleanup defer above, deliberately: this starts extension
	// processes, and a failure between starting them and installing the defer
	// would return without stopping them, leaving them running or killed
	// abruptly without their shutdown hooks. Nothing before this point needs
	// them.
