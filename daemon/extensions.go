...
//
// It is empty until the engine defines its first point. Socket exposure is not
// listed here and never will be: service.grpc is resolved locally rather than
// over the wire (its method takes a [grpc.ServiceRegistrar]), so it has no
// ClientPoint to register.
...
//
// It is empty: the daemon defines no points of its own yet, so it has nothing
// to provide. A built-in is registered here, and because a built-in yields to
// an installed extension on a single-provider point, whatever is listed here is
// the stock behavior an installed extension replaces by simply providing the
// point.
